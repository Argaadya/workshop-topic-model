...
The `covid_news` is a 'data.frame' or a table with 7857 rows and 6 columns. The name of each column can be seen on the left side (authors, title, etc.). The `chr` text means that the column has the data type of character, followed by the content of the column.

### [OPTIONAL] Data Type in R
...
```{r}
c(1:5, 20, 15, 10.5, 1.3)
```

* Integer

Integer is where you store integer values. Integer data can also be applied with arithmatical function such as addition, substraction and other mathematical function.
...
Try applying function to samples of the data before using mutate to our data. For example, here I use `quarter()` function to get the information about at what quarter of the year the news is published.
...
Before we proceed to do text analysis, we will do some practice to make you more familiar with data wrangling. Our goal in this part is to check the daily number of news related to Covid-19. What we gonna do is simply counting the number of articles at any given day.
...
Try to visualize the number of news on each day (publish_day). First, you need to create an object called day_count that store information of number of news on each day of the week.

```{r}
day_count <- covid_news %>% 
  count(publish_day, 
        name = "frequency" 
        )
...
Now you can create the plot to visualize the data. Instead of using line chart, try use bar chart by change the `geom_line()` with another function that can create a bar chart with ggplot2, which you can look for in the cheatsheet. You can also change the title of the axis and the plot to better represent the data.
...
ggplot(data = day_count, 
  ...
  labs(title = "Number of Covid-19 News by Day", 
       x = "Publish Day", 
       y = "Frequency", 
       caption = "Source: CBC News"
       ) + 
...
There are many package to do text mining, such as `tm`, `quanteda`, and `tidytext`. For this course, we will only use the `tidytext` due to its interpretability and easier to write. If you are interested in learning more about `tidytext`, you can visit the wonderful book written by Julia Silge[^5].

For this lesson, we will only use the news published in December, January and February to reduce the size and computation time. 

```{r}
covid_news <- covid_news %>% 
  filter(publish_month %in% c("Dec", "Jan", "Feb"))
...
```{r}
read.csv("https://raw.githubusercontent.com/nasalsabila/kamus-alay/master/colloquial-indonesian-lexicon.csv") %>% 
  head(10)
```

### Removing Hashtag (#rstats) and URL
...
```{r}
read.delim("https://raw.githubusercontent.com/stopwords-iso/stopwords-id/master/stopwords-id.txt", header = F) %>% 
  head(10)
```
...
One of the most famous stemming algorithm is Porter's algorithm[^6], which have several rules that will stem a word into its basic form. Since the algoithm only consists of crude rules, some words may lost its form, such as the word "coronavirus" become "coronaviru".
...
LDA will return several output for us. Below are some important attribute acquired from the LDA Model:
...
We will try to find the optimal number of topics by finding the average probabilistic coherence for several number of topics, ranging from k = 5 and k = 10 to k = 100 with interval of 10. To speed up the computation, we will only use 500 sampling iterations for the sake of illustration since higher number of iterations can run for hours.
...
The optimal number of topics can be chosen by picking the number of topics that give the highest average coherence. There are also other methods to evaluate the topic model. It will be too much to discuss them on this article. You can visit Julia Silge blogpost[^9] to see some of the evaluation metrics.

### Extrinsic Measures

Model performance toward a specific task, such as text classification. If the topics is regarded as a feature for classification model, we can use accuracy or any other classification metrics to check if the topic model is good enough to do the job. You can check the bottom part of my other article[^10] to see how LDA can be used for dimensionality reduction for text classification if you have learned about machine learning. 
...
[^5]: [Text Mining with R](https://www.tidytextmining.com/)
[^6]: [The Porter Stemming Algorithm](https://tartarus.org/martin/PorterStemmer/)
[^7]: [Latent Dirichlet Allocation](http://www.jmlr.org/papers/volume3/blei03a/blei03a.pdf)
[^8]: [Surveying A Suite of Algorithms that Offer a Solution to Managing Large Document Archives](http://www.cs.columbia.edu/~blei/papers/Blei2012.pdf)
[^9]: [Training, Evaluating, and Interpreting Topic Models](https://juliasilge.com/blog/evaluating-stm/)
[^10]: [Topic Modeling with Latent Dirichlet Allocation (LDA)](https://algotech.netlify.app/blog/topic-modeling-lda/)